...
def run(benchmark, opts):
    ...
    for __ in range(opts.steps):
        ...
        average_batches_per_sec += (opts.batches_per_step /
                                    duration)/opts.steps
        report_string = "{:<8.3} sec/step.".format(duration)
        report_string += "   " + benchmark.iteration_report(opts, duration)
        
        print(report_string)

        duration = duration / opts.batches_per_step

        tflop = 2 * 4 * opts.timesteps * opts.hidden_size * opts.hidden_size * 1 * opts.batch_size + 2 * 4 * opts.timesteps * opts.hidden_size * opts.hidden_size * 1 * opts.batch_size
        tflops = tflop / duration / 10**12
